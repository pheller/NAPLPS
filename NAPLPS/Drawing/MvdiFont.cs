...
public static class MvdiFont
{
    ...
    public static double ProdigyProportionalAdvanceNorm(char c, double charSizeX)
    {
        ...
        int g;
        // At small sizes (k=5..7) the advance is genuinely PER-GLYPH, not a function of the
        // AdvanceWidth class: narrow-ink punctuation (',' ';') advances less and '#'/'Q' more than
        // their class. Those deviations vanish for k>=8, where the AW-class table is exact.
    ...
    // Measured from the reference render; supersedes the AW-class table at these sizes where a few glyphs
    // deviate. Rows: k=5, k=6, k=7.
    ...
    public static readonly Glyph[] Glyphs = Build();
