...
$baselinesDir = Join-Path (Join-Path (Join-Path $repoRoot "NAPLPSTests") "Visual") "Baselines"

# Find actuals directory - check common locations
$actualsDir = $null
$candidates = @(
    (Join-Path (Join-Path (Join-Path (Join-Path (Join-Path $repoRoot "NAPLPSTests") "bin") "Debug") "net10.0") "VisualRegression" | Join-Path -ChildPath "Actuals"),
    (Join-Path (Join-Path (Join-Path (Join-Path (Join-Path $repoRoot "NAPLPSTests") "bin") "Release") "net10.0") "VisualRegression" | Join-Path -ChildPath "Actuals")
)
